...
class Competitors_Ajax_OfflineSyncHandler {
    ...
    public static function handle_sync() {
        ...
        foreach ( $scores as $score ) {
            ...
            if ( $existing && ! empty( $client_scored_at ) ) {
                $server_time = strtotime( $existing['scored_at'] );
                $client_time = strtotime( $client_scored_at );

                if ( $server_time && $client_time && $server_time >= $client_time ) {
                    // Server score is same age or newer — skip
                    $results[] = array(
                        'competitor_id'      => $competitor_id,
                        'competition_roll_id' => $competition_roll_id,
            ...
            // Client is newer or no server score exists — save
            $left_group  = (int) ( $score['left_group'] ?? 0 );
            $right_group = (int) ( $score['right_group'] ?? 0 );
            $left_score  = (float) ( $score['left_score'] ?? 0 );
            $right_score = (float) ( $score['right_score'] ?? 0 );
